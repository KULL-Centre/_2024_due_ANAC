...
def optTauC(name,prot,chi2_results,rs_results,tau_c_range, weights, delay):
    chi2list = []
    for tc in tau_c_range:
        ...
        for label in prot.labels:
            ...
            gamma_2_av[measured_resnums] = np.ma.average(gamma_2, axis=0, weights=weights).data
            if prot.obs == 'ratio':
...
time0 = time.time()
tau_c, chi2_pre = optTauC(args.name,proteins.loc[args.name],chi2_results,rs_results,tau_c_range,weights,delay)
PREs = {}
